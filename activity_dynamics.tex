...
\caption{\textbf{In a wide network doing weight descent, activity changes closely match activity-space gradient descent.}  Top row: width\,=\,8.  Middle row: width\,=\,48.  Left: normalised $\Theta_{ik}$ heatmaps (white = uncorrelated, blue = correlated).  Centre panels: predicted vs.\ actual $\Delta A$ for the chain-rule identity (Eq.\,2), the full kernel (Eq.\,3), and the diagonal approximation (Eq.\,5).  Bottom row: $R^2$ and loss over training.  At width\,=\,8 the diagonal approximation fails ($R^2<0$); at width\,=\,48 it is near-perfect.}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.55\textwidth]{fig_width_sweep.pdf}
\caption{\textbf{The wider the network, the more faithfully activity changes track the activity-space loss gradient.}  $R^2$ of the diagonal approximation (Eq.\,5) as a function of hidden-layer width.  At width\,=\,4 the approximation fails; by width\,$\sim$\,32 it is reliable ($R^2>0.9$).  Shaded regions: $\pm1$ s.d.\ across seeds.}
\end{figure}
